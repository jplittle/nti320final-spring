...
cd /tmp
#clone repository-------------------------------
git clone https://github.com/mnichols-github/nti320final-spring-spring.git
...
cp /tmp/nti320final-spring/config.php /etc/phpldapadmin/config.php
...
changetype: modify
replace: olcSuffix
olcSuffix: dc=nti320,dc=local
\n
dn: olcDatabase={2}hdb,cn=config
changetype: modify
replace: olcRootDN
olcRootDN: cn=ldapadm,dc=nti320,dc=local
\n
dn: olcDatabase={2}hdb,cn=config
changetype: modify
replace: olcRootPW
...
changetype: modify
replace: olcAccess
olcAccess: {0}to * by dn.base="gidNumber=0+uidNumber=0,cn=peercred,cn=external, cn=auth" read by dn.base="cn=ldapadm,dc=nti320,dc=local" read by * none' > monitor.ldif
ldapmodify -Y EXTERNAL  -H ldapi:/// -f monitor.ldif
# Generate certs-------------------------------------------
openssl req -new -x509 -nodes -out /etc/openldap/certs/nti320ldapcert.pem -keyout /etc/openldap/certs/nti320ldapkey.pem -days 365 -subj "/C=US/ST=WA/L=Seattle/O=SCC/OU=IT/CN=nti320.local"
chown -R ldap. /etc/openldap/certs/nti*.pem
# Use Certs in LDAP----------------------------------------
echo -e "dn: cn=config
changetype: modify
replace: olcTLSCertificateKeyFile
olcTLSCertificateKeyFile: /etc/openldap/certs/nti320ldapkey.pem
\n
dn: cn=config
changetype: modify
replace: olcTLSCertificateFile
olcTLSCertificateFile: /etc/openldap/certs/nti320ldapcert.pem" > certs.ldif
...
echo -e "dn: dc=nti320,dc=local
dc: nti320
objectClass: top
objectClass: domain
\n
dn: cn=ldapadm ,dc=nti320,dc=local
objectClass: organizationalRole
cn: ldapadm
description: LDAP Manager
\n
#create main Group Group------------------
dn: ou=Group,dc=nti320,dc=local
objectClass: organizationalUnit
ou: Group
\n
#create child Group-A---------------------------
dn: cn=Group-A,ou=Group,dc=nti320,dc=local
cn: Group-A
gidnumber: 500
...
dn: cn=Group-B,ou=Group,dc=nti320,dc=local
cn: Group-B
gidnumber: 501
objectclass: posixGroup
objectclass: top
...
dn: ou=People,dc=nti320,dc=local
objectClass: organizationalUnit
ou: People
\n
#create child People <users>------------------
dn: cn=name-0 lname-0,ou=People,dc=nti320,dc=local
cn: name-0 lname-0
gidnumber: 500
givenname: name-0
...
\n
dn: cn=name-1 lname-1,ou=People,dc=nti320,dc=local
cn: name-1 lname-1
gidnumber: 500
givenname: name-1
...
\n
dn: cn=name-2 lname-2,ou=People,dc=nti320,dc=local
cn: name-2 lname-2
gidnumber: 500
givenname: name-2
...
userpassword: {SHA}W6ph5Mm5Pz8GgiULbPgzG37mj9g=
\n
dn: cn=name-3 lname-3,ou=People,dc=nti320,dc=local
cn: name-3 lname-3
gidnumber: 500
givenname: name-3
...
userpassword: {SHA}W6ph5Mm5Pz8GgiULbPgzG37mj9g=
\n
dn: cn=name-4 lname-4,ou=People,dc=nti320,dc=local
cn: name-4 lname-4
gidnumber: 500
givenname: name-4
...
userpassword: {SHA}W6ph5Mm5Pz8GgiULbPgzG37mj9g=
\n
dn: cn=name-5 lname-5,ou=People,dc=nti320,dc=local
cn: name-5 lname-5
gidnumber: 500
givenname: name-5
...
ldapadd -x -W -D "cn=ldapadm,dc=nti320,dc=local" -f base.ldif -y /root/ldap_admin_pass
#restart httpd
